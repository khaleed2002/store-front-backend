...
export class UserModel{
    async index():Promise<User[]>{
        try{
            ...
            throw new Error(
                `Failed to get users with following error: ${err}`
            );
        }
    };
    ...
    async show(user_id:number):Promise<User>{
        try{
            const conn = await client.connect();
            const sql ='SELECT username,id,firstname,lastname FROM users WHERE id=($1)';
            const result= await conn.query(sql,[user_id]);
            conn.release();
            return result.rows[0];
        } catch(err){
            throw new Error(
                `Failed to get user with following error: ${err}`
            );
        }
    };
    async create(user:User):Promise<User>{
        try{
            ...
            const hash = bcrypt.hashSync((user.password as unknown) as string+pepper,parseInt((SALT_ROUNDS as unknown)as string));
            const result= await conn.query(sql,[user.username,user.firstname,user.lastname,hash]);
            conn.release();
            return result.rows[0];
        } catch(err){
            throw new Error(
                `Failed to create user with following error: ${err}`
            );
        }
    };
    ...
    async delete(user_id:number):Promise<User>{
        try{
            ...
            throw new Error(
                `Failed to delete user with following error: ${err}`
            );
        }
    }
