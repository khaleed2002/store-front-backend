...
export class OrderModel {
  async index(): Promise<Order[]> {
    try {
      ...
      throw new Error(`Failed to get orders with following error:${err}`);
    }
  }
  async currentOrder(user_id: number) {
    try {
      const conn = await client.connect();
      ...
      const sql =
        'SELECT order_id,product_id,quantity,status FROM orders o FULL OUTER JOIN orders_products o_p ON o.id=o_p.order_id WHERE order_id=$1';
  ...
  async create(order: Order): Promise<Order> {
    try {
      ...
      throw new Error(`Failed to create orders with following error:${err}`);
    }
  }
